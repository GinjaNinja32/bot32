...
do_extras(Tokens, Reply, Ping) ->
	...
	Nick = util:regex_escape(config:require_value(config, [bot, nick])),
	Aliases = lists:map(fun util:regex_escape/1, config:require_value(config, [bot,names])),
	case re:run(String, ["\\b(", util:binary_join([Nick|Aliases],<<"|">>), ")s?\\b"], [{capture, none}, caseless]) of
		match ->
